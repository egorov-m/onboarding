...
async def get_steps(
    ...
    return await BoardStepsApiService.get_steps(board_id, user.id, is_include_blobs)
...
async def get_step_by_index(
    ...
    return await BoardStepsApiService.get_step_by_index(board_id, index, user.id, is_include_blobs)
...
async def move_step_by_index(
    ...
    return await BoardStepsApiService.move_step_by_index(board_id, index, new_index, user.id, is_include_blobs)
...
async def get_step(
    ...
    return await BoardStepsApiService.get_step_by_id(str(board_step_id), user.id, is_include_blobs)
...
async def update_step(
    ...
    return await BoardStepsApiService.update_step(str(board_step_id), update_req, user.id, is_include_blobs)


@board_steps_api.delete("/{board_step_id}", status_code=HTTPStatus.NO_CONTENT)
async def delete_step(board_step_id: UUID, user: AuthUserData = Depends(get_auth_user())):
    await BoardStepsApiService.delete_step(str(board_step_id), user.id)
